  ...
  static void _catchError(e, stackTrace, OnError onError) {
    ...
    if (e is DioError) {
      if (e.type == DioErrorType.connectTimeout ||
        ...
        onError('Falló la conexión con el Servidor', {});
        ...
        if (response != null) {
          ...
          if (data != null && data is Map<String, dynamic>) {
            showSessionDialog();
            onError('No se pudo obtener respuesta: ${e.message}', data);
            return;
          }
        }
        onError('No se pudo obtener respuesta: ${e.message}', {});
      } else {
        onError('Respuesta cancelada', {});
      }
    } else {
      onError(e?.toString() ?? 'Ocurrió un error desconocido', {});
    }
  }
  ...
  static authenticate({
    ...
    request(
      ...
      onResponse: (response) {
        switch(response){
          case null:
            showWarning('Verifique las configuraciones de conexión');
          break;
          default:
            if (response['uid'] == false){
              showWarning('Usuario o Contraseña incorrecto');
            }else{
